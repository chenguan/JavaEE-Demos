...
public class ChatService implements Runnable {
	private static final Map service = new HashMap();// 会议室服务,系统中的当前会议室存放到该Map中
	private static final int maxServices = 10;// 可以同时开的最大会议室数
	private static final int maxMsgLine = 50;// 每次最多读取的数据总数
	private static final long maxHistorySize=1024*1024;//每一个聊天记录文件的最大数量
	 ...
	 * @return
	 */
	public static ChatService create(Serializable name) {
	 ...
	 * @return
	 */
	public static boolean close(Serializable name) {
	 ...
	 * @return
	 */
	public static ChatService get(Serializable name) {
		if (service.containsKey(name)) {
			return (ChatService) service.get(name);
		} else {
			return null;
		}
	}
	...
	public boolean talk(Chat chat) {
		//chat.setContent(CommUtil.convert2unicode(chat.getContent().replace('"','“'), false));
	...
	public boolean join(ChatUser user) {
		...
		if (canJoin(user)) {
			...
			//talk(geneSystemMsg(user.getUserName() + "进入教室!"));
			ret = true;
	...
	public boolean exit(ChatUser user) {
		//talk(geneSystemMsg(user.getUserName() + "退出了教室！"));
	...
	public boolean canJoin(ChatUser user) {
		...
		} else// 判断访问权限
		{
			...
			{
				// ret= user.getScore().intValue()>new
				// Integer(this.getVrvalue()).intValue();
			} else if (vrType.equals("member")) {
				// ret=user.getTypes()!=null && (!user.getTypes().equals("")) &&
				// user.checkUserRole("member");
			} else if (vrType.equals("admin")) {
				//ret=UserContext.isAdmin();
				// ret=user.getTypes()!=null && (!user.getTypes().equals("")) &&
				// user.checkUserRole("admin");
			}
			else if(vrType.equalsIgnoreCase("vip")){
				//ret=UserContext.isVip();
			}			
		}
		return ret;
	}
	...
	public List getNewestMsg(ChatUser user, String lastReadId)// 根据上次读出的Id号读出聊天信息
	{
		...
		for (int i = msgs.size() - 1, j = 0; i >= 0 && j < maxMsgLine; i--, j++) {
			Chat info = (Chat) msgs.get(i);
			if ((now - info.getVdate().getTime() > this.interval)
					|| (info.getId().equals(lastReadId))) {
				break;
			}
			ret.add(0, info);
		}
	...
	public void flashChatMsg() {
		...
		for (int i = 0; i < delList.size(); i++) {
			Chat info = (Chat) msgs.get(i);
			/*s.append("<font color=blue>" + info.getSender()
					+ "</font>对<font color=blue>" + info.getReciver()
					+ "</font>说:" + info.getContent() + "(" + info.getVdate()
					+ ")\r\n");*/
			s.append("<div><div class='yourMsg'><b>"+info.getSender()+"</b> 对 <b>"+info.getReciver()+"</b>　"+CommUtil.formatDate("yyyy-M-d H:m:s", info.getVdate())+"</div><div>　"+
			info.getContent()+"</div></div>");
		}
		if (s.length() > 1) {
	...
	public void flashChatUser() {
		...
		for (int i = 0; i < users.size(); i++) {
			...
			if ((now - info.getLastAccessTime().getTime()) > this.interval) {
				delList.add(0, info);
			}
		}
		for (int i = 0; i < delList.size(); i++) {
	...
	private void saveToFile(String s) {
		try {
			File dir=new File(filePath + "/" + this.title + "/"
					+ df.format(new Date()) + "/");
			File file =new File(dir,"1.txt");
			if (!file.getParentFile().exists()) {
				file.getParentFile().mkdirs();
			}
			file =new File(dir,(dir.list().length>0?dir.list().length:1)+".txt");
			if(file.exists() && file.length()>maxHistorySize)
			file=new File(dir,(dir.list().length+1)+".txt");			
	...
	public void flashAll() {
		...
		for (int i = 0; i < msgs.size(); i++) {
			Chat info = (Chat) msgs.get(i);			
			//s.append(info.getSender() + "对" + info.getReciver() + "说:"+ info.getContent() + "(" + CommUtil.formatDate("yyyy-M-d H:m:s", info.getVdate()) + ")\r\n");
			s.append("<div><div class='yourMsg'><b>"+info.getSender()+"</b> 对 <b>"+info.getReciver()+"</b>　"+CommUtil.formatDate("yyyy-M-d H:m:s", info.getVdate())+"</div><div>　"+
			info.getContent()+"</div></div>");
		}
		msgs.clear();
